...
export default function FinCastSelfDemo() {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- narrationSteps stable; handleRun defined below
  ...
  const scenarioData = useMemo(() => {
    ...
    const retireLater = runScenario(
      Math.min(retireAge + 2, 75),
      annualReturn,
      retirementSpending
    );
    const spendLess = runScenario(
      retireAge,
      annualReturn,
      Math.max(retirementSpending - 10000, 40000)
    );
    const stressRet = runScenario(
      retireAge,
      Math.max(annualReturn - 1, 0.1),
      retirementSpending
    );
  ...
  const scenarioVoiceLabels = ["Base case", "Retire later", "Spend less", "Stress return"];
  ...
  #fincast-print-summary, #fincast-print-summary * { visibility: visible !important; }
  #fincast-print-summary {
    ...
    width: 100% !important;
      ...
      {printPortalReady &&
        createPortal(
          <div id="fincast-print-summary" style={{ display: "none" }} aria-hidden="true">
            <PrintSummary
      ...
      <div className="min-h-screen bg-slate-50 text-slate-950 p-4 md:p-8 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          >
            <Card
              ref={hookCardRef}
              className={`rounded-2xl shadow-sm border-slate-200 bg-white ${demoStep === 0 ? "ring-4 ring-slate-300" : ""}`}
            >
              <CardContent className="p-7 md:p-10 space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-lg md:text-xl text-slate-600 max-w-2xl">
                    FinCast Reva helps advisors turn retirement uncertainty into a simple visual
                ...
                <div className="rounded-2xl bg-slate-950 text-white p-5 space-y-3">
                  <div className="text-sm text-slate-300">
                    {isAutoPlaying ? "Auto-playing" : "Narrated Demo"} · Step {demoStep + 1} of{" "}
                    {narrationSteps.length}
                  </div>
                  <div className="text-xl font-semibold">{narrationSteps[demoStep].title}</div>
                  <p className="text-slate-200 text-sm leading-relaxed">
                    &ldquo;{narrationSteps[demoStep].voice}&rdquo;
                  </p>
                </div>
            ...
            <Card
              ref={resultCardRef}
              className={`rounded-2xl shadow-sm border-slate-200 bg-white overflow-hidden ${demoStep === 2 ? "ring-4 ring-slate-300" : ""}`}
            >
              <CardContent className="p-6 md:p-8 space-y-5">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <div className="text-sm text-slate-500">Instant Result</div>
                    <div className={`text-2xl md:text-3xl font-semibold ${riskTone}`}>
                      {resultMessage}
                    </div>
                  </div>
                  ...
                  <div className="h-[48rem] w-full relative">
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={displayData} margin={{ top: 12, right: 18, left: 8, bottom: 8 }}>
                        ...
                        <XAxis
                          dataKey="age"
                          tick={{ fontSize: 17, fill: "#0f172a", fontWeight: 600 }}
                          stroke="#475569"
                          domain={[Number(ageNow), 100]}
                          type="number"
                        />
                        <YAxis
                        ...
                        <Line
                          type="monotone"
                          dataKey="Base Case"
                          name={`Base Case ($${(retirementSpending / 1000).toFixed(0)}k/yr spend)`}
                          stroke="#1e3a8a"
                          strokeWidth={5}
                          dot={false}
                        ...
                        <Line
                          type="monotone"
                          dataKey="Retire Later"
                          name={`Retire Later (age ${Math.min(retireAge + 2, 75)})`}
                          stroke="#047857"
                          strokeWidth={4}
                          dot={false}
                          strokeDasharray="8 4"
                        ...
                        <Line
                          type="monotone"
                          dataKey="Spend Less"
                          name={`Spend Less ($${(Math.max(retirementSpending - 10000, 40000) / 1000).toFixed(0)}k/yr spend)`}
                          stroke="#6d28d9"
                          strokeWidth={4}
                          dot={false}
                          strokeDasharray="8 4"
                        ...
                        <Line
                          type="monotone"
                          dataKey="Stress Return"
                          name={`Stress Return (${Math.max(annualReturn - 1, 0.1).toFixed(1)}% return)`}
                          stroke="#c2410c"
                          strokeWidth={4}
                          dot={false}
                          strokeDasharray="8 4"
                ...
                <div className="rounded-2xl bg-slate-100 border border-slate-200 p-5 flex items-center gap-3 text-slate-900">
                  ...
                  <p className="text-xl font-semibold leading-snug">
                    Advisor talking point: &ldquo;This is the moment clients understand retirement
                    survivability without reading a 40-page report.&rdquo;
                  </p>
                </div>
              </CardContent>
            </Card>
          ...
          <div className="grid lg:grid-cols-[0.8fr_1.2fr] gap-6">
            <Card
              ref={inputsCardRef}
              className={`rounded-2xl shadow-sm border-slate-200 bg-white ${demoStep === 1 ? "ring-4 ring-slate-300" : ""}`}
            >
              <CardContent className="p-6 space-y-5">
                ...
                <div className="space-y-3 pt-1">
                  <SliderRow
                    ...
                    setValue={setAnnualContributions}
                    min={0}
                    max={100000}
                    step={1000}
                    prefix="$"
                ...
                <div className="space-y-3 pt-2">
                  ...
                  <SliderRow
                    ...
                    setValue={setRetirementSpending}
                    min={50000}
                    max={500000}
                    step={5000}
                    prefix="$"
                  ...
                  <SliderRow
                    ...
                    setValue={setOtherRetirementIncome}
                    min={0}
                    max={100000}
                    step={1000}
                    prefix="$"
            ...
            <Card
              ...
              className={`rounded-2xl shadow-sm border-slate-200 bg-white ${demoStep === 3 || demoStep === 4 ? "ring-4 ring-slate-300" : ""}`}
            >
              <CardContent className="p-6 md:p-8 space-y-6">
                <div>
                  <h2 className="text-2xl md:text-3xl font-semibold">Scenario Conversation</h2>
                  <p className="text-slate-600 mt-2">
                    Move one assumption and the client immediately sees how the retirement picture
                    changes.
                  </p>
                </div>
                ...
                <div className="rounded-2xl bg-slate-950 text-white p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-5">
                  <div>
                    ...
                    <div className="text-2xl font-semibold">
                      See how FinCast Reva works in a client meeting.
                    </div>
        ...
        <div className="max-w-7xl mx-auto mt-6 rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          <button
            type="button"
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                ...
                <div className="text-sm text-slate-500">
                  Step-by-step talking guide for using FinCast Reva in a client meeting
                </div>
              </div>
            </div>
            {scriptOpen ? (
            ...
            <div className="border-t border-slate-100 px-7 py-6 space-y-6">
                  ...
                  <div className="space-y-3">
                    ...
                    {step.tip && (
                      <div className="rounded-xl bg-amber-50 border border-amber-100 p-3 text-sm text-amber-800 leading-relaxed">
                        <span className="font-semibold">RIA tip: </span>
                        {step.tip}
                      </div>
                    )}
                  </div>
              ...
              <div className="rounded-2xl bg-slate-50 border border-slate-200 p-5 mt-2">
                ...
                <p className="text-xs text-slate-500 mb-4">
                  When your client pushes back, here&apos;s how to respond.
                </p>
                <div className="space-y-3">
                  {OBJECTIONS.map((o, i) => (
...
const MEETING_SCRIPT = [
  {
    ...
    timing: "0:00 – 0:30",
    script:
      "I want to show you something before we get into the details. I'm going to ask you a straightforward question that most advisors avoid. Ready? Will you run out of money in retirement?",
  ...
  },
  {
    ...
    timing: "0:30 – 1:00",
    script:
      "I'm not going to hand you a 40-page report. What I'm going to do is put four numbers in, and in about ten seconds you'll see the picture clearly. We can then talk about what it means and what we can do about it.",
  ...
  },
  {
    ...
    timing: "1:00 – 2:00",
    script:
      "Tell me — how old are you today? And when are you hoping to retire? Good. What would you say your retirement savings are right now, ballpark? And roughly what do you think you'd want to spend each year in retirement?",
  ...
  },
  {
    ...
    timing: "2:00 – 2:30",
    script:
      "Here's the picture. This line is your projected portfolio balance from today through age 100. This dashed line is when you retire. If the line stays above zero the whole way, you're in good shape. If it dips below — that's the conversation we need to have.",
  ...
  },
  {
    ...
    timing: "2:30 – 3:30",
    script:
      "Now watch what happens if we make just one small change. What if you worked two more years — retired at [X] instead of [Y]? Look at that. The picture changes significantly. That's two years of work that could mean a decade of security.",
  ...
  },
  {
    ...
    timing: "3:30 – 5:00",
    script:
      "This is a simplified model — it doesn't include Social Security, taxes, or everything we'd put into a full plan. But this is the foundation. What's jumping out at you right now? What questions does this raise for you?",
  ...
  },
  {
    ...
    timing: "5:00 – 5:30",
    script:
      "What I'd like to do is take these numbers, build out a full plan with all the details, and come back to you with a real picture and a set of options. Does that sound like a useful next step?",
...
const OBJECTIONS = [
  {
    ...
    response:
      "They are simple — intentionally. This is the 30,000-foot view. The full plan includes taxes, Social Security, healthcare, and everything else. This is just the starting conversation.",
  },
  {
    objection: "I'm not sure what my retirement spending will be.",
    response:
      "That's completely normal. Let's use your current spending as a starting point and then stress-test it a little. The goal right now is to understand the range, not pin down the exact number.",
  },
  {
    objection: "The market could be different from that return assumption.",
    response:
      "Absolutely — let me show you what happens if we drop the return by a couple of percent. That's exactly why we stress-test. You want to know where the edges are before you need them.",
  },
  {
    objection: "I thought I was in better shape than this.",
    response:
      "You may well be — this is a baseline. The full plan typically looks better because it accounts for Social Security and other income. Let's build that picture together.",
  },
];
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div>
          ...
          >
            FinCast Reva — Retirement Projection Summary
            {clientName ? `: ${clientName}` : ""}
          </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "200px 1fr", gap: 14 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          ...
          >
            {[
              {
                label: `Base Case ($${(retirementSpending / 1000).toFixed(0)}k/yr spend)`,
                color: "#1e3a8a",
                dashed: false,
              },
              {
                label: `Retire Later (age ${Math.min(retireAge + 2, 75)})`,
                color: "#047857",
                dashed: true,
              },
              {
                label: `Spend Less ($${(Math.max(retirementSpending - 10000, 40000) / 1000).toFixed(0)}k/yr spend)`,
                color: "#6d28d9",
                dashed: true,
              },
              {
                label: `Stress Return (${Math.max(annualReturn - 1, 0.1).toFixed(1)}% return)`,
                color: "#c2410c",
                dashed: true,
              },
            ].map((item) => (
          ...
          >
            This projection is based on the assumptions entered above and uses a straight-line return
            model. It does not account for taxes, investment fees, or income sources beyond those
            entered. This document is for illustrative and discussion purposes only and does not
